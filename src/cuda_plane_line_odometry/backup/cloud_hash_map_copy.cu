...
#define NTHREADS_BUILD_MAP  32 // 256
#define NTHREADS_KNN_SEARCH 32 // 256

// ----------

__device__ __host__ GridKey key_func(const float4& point) {
...
__device__ __host__ int hash_func(const GridKey& key) {
...
__global__ void kernel_5_non_empty(
    int num_unique_hash,
    int* num_hash_,
    int* num_keys_,
    const int* unique_by_hash_hash_,
...
__global__ void kernel_6_non_empty(
    int num_unique_keys,
    const int* unique_by_key_key_idx_,
    const int2* unique_by_key_point_start_end_,
    const float4* point_,
...
__global__ void kernel_rearrange_by_idx_0(
    int num_map_points,
    const int* src_idx_,
...
__global__ void kernel_rearrange_by_idx_1(
    int num_unique_keys,
    const int* src_idx_,
...
CUDACloudHashMap::CUDACloudHashMap(
    ...
    max_num_keys_per_hash(max_num_keys_per_hash_), 
    max_num_points_per_key(max_num_points_per_key_), 
    max_insertion_size(max_insertion_size_)
{
    cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);

    unsigned int mod                   = max_num_hashes;
    unsigned int key_bucket_max_size   = mod * max_num_keys_per_hash;
    unsigned int point_bucket_max_size = key_bucket_max_size * max_num_points_per_key;
...
void CUDACloudHashMap::InsertV2(const thrust::host_vector<float4> &host_point) {
    ...
    );
    {
        dim3 num_threads_per_block(NTHREADS_BUILD_MAP);
        dim3 num_blocks_per_grid((num_inserted_points + NTHREADS_BUILD_MAP - 1) / NTHREADS_BUILD_MAP);
        kernel_rearrange_by_idx_0<<<num_blocks_per_grid, num_threads_per_block, 0, stream>>>(
            num_inserted_points,
...

    {
        ...
        kernel_4_0<<<num_blocks_per_grid, num_threads_per_block, 0, stream>>>(
            num_unique_keys_in_inserted_points,
            num_inserted_points,
    ...
    );
    {
        dim3 num_threads_per_block(NTHREADS_BUILD_MAP);
        dim3 num_blocks_per_grid((num_unique_keys_in_inserted_points + NTHREADS_BUILD_MAP - 1) / NTHREADS_BUILD_MAP);
        kernel_rearrange_by_idx_1<<<num_blocks_per_grid, num_threads_per_block, 0, stream>>>(
            num_unique_keys_in_inserted_points,
            thrust::raw_pointer_cast(&(dev_src_idx[0])),
    ...
    // update key bucket
    {
        ...
        if (empty) {
            ...
            kernel_5_non_empty<<<num_blocks_per_grid, num_threads_per_block, 0, stream>>>(
                num_unique_hash_in_inserted_points,
    ...
    // update point bucket
    {
        ...
        if (empty) {
            ...
            kernel_6_non_empty<<<num_blocks_per_grid, num_threads_per_block, 0, stream>>>(
                num_unique_keys_in_inserted_points,
                thrust::raw_pointer_cast(&(dev_unique_by_key_key_idx[0])),
                thrust::raw_pointer_cast(&(dev_unique_by_key_point_start_end[0])),
                thrust::raw_pointer_cast(&(dev_point[0])),
