...
#ifndef LIO_SAM_CUDA_COL_PIV_HOUSEHOLDER_QR_V2_CUH
#define LIO_SAM_CUDA_COL_PIV_HOUSEHOLDER_QR_V2_CUH
...
{
    Eigen::Matrix<float, NROW, 3> m_vectors;
...
{
    static const int rows = NROW;
    static const int cols = 3;
    static const int size = 3;	// diag size

    Eigen::Matrix<float, NROW, 3> m_qr;
    ...
    Eigen::Matrix<float, NROW, 1> m_c;
    Eigen::Matrix<float, 1, 1> m_rhs_workspace;
    Eigen::Vector3f m_solution;

    // funcs
    __device__ ColPivHouseholderQRNx3();
    __device__ explicit ColPivHouseholderQRNx3(const Eigen::Matrix<float, NROW, 3>& A);
...
template<unsigned int NROW>
__device__ ColPivHouseholderQRNx3<NROW>::ColPivHouseholderQRNx3(const Eigen::Matrix<float, NROW, 3>& A)
{
    Compute(A);
}

template<unsigned int NROW>
__device__ Eigen::Vector3f ColPivHouseholderQRNx3<NROW>::Solve(const Eigen::Matrix<float, NROW, 1>& rhs)
{
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::SetUpHouseholderSequence()
{
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::ApplyHouseholderOnTheLeftToRHS(
        int actual_k,
        int dstStart,
        int start,
        int dst_num_cols	// inputIsIdentity ? dstStart : dst.cols()
) {
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::ApplyHouseholderSequenceOnTheLeftToRHS(const Eigen::Matrix<float, NROW, 1>& rhs)
{
    m_c = rhs;
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::Compute(const Eigen::Matrix<float, NROW, 3>& A)
{
    m_qr = A;
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::MakeHouseHolderInPlace(int col_idx, float& beta) {
...
template<unsigned int NROW>
__device__ void ColPivHouseholderQRNx3<NROW>::ApplyHouseholderOnTheLeft(int col_idx)
{
...
#endif //LIO_SAM_CUDA_2_COL_PIV_HOUSEHOLDER_QR_BACKUP_2_CUH