...
public :
    ...
    GPUMemBlock(int max_nrows, int max_ncols) {
        ...
        device_ptr = nullptr;
        cuda_status = cudaMalloc((void**)&device_ptr, sizeof(T) * max_num_rows * max_num_cols); cudaStreamSynchronize(0);
        assert(cudaSuccess == cuda_status);
        cuda_status = cudaMemset(device_ptr, 0, sizeof(T) * max_num_rows * max_num_cols); cudaStreamSynchronize(0);
        assert(cudaSuccess == cuda_status);
    }
...
public :
    ...
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> ToEigen() {
        ...
        for(int i = 0; i < cols(); i++) {
            cudaMemcpy(eigen_matrix.col(i).data(), GetDevicePtr() + i * ld(), sizeof(T) * rows(), cudaMemcpyDeviceToHost); cudaStreamSynchronize(0);
        }
