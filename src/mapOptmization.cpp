...
public:
    ...
    mapOptimization()
    {
        ...
        max_num_scan_in_gpu_local_map = 0;
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        std::set<int> remove_dup; remove_dup.clear(); // remove duplicates in cloudToExtract
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) {
                ...
                if (!useGPULocalMap) {
                    ...
                    *laserCloudSurfFromMap   += laserCloudSurfTemp;
                    ...
                    useGPULocalMap &&
                    gpu_local_map_scan_idx_set.find(thisKeyInd) == gpu_local_map_scan_idx_set.end()
                ) {
                    gpu_local_map_scan_idx_set.insert(thisKeyInd);
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            printf(
                "transformTobeMapped[0~5] : %f , %f , %f , %f , %f , %f \n",
    ...
    void scan2MapOptimizationWithCUDA()
    {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            for(int i = 0; i < laserCloudSurfLastDS->size(); i++) {
                surf_pts_3d[i] = { 
                    laserCloudSurfLastDS->points[i].x,
            ...
            std::cout << "duration ms : one frame : " << cur_dur_one_frame << std::endl;

            // tf::Quaternion q; q.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
            // printf("q : %f , %f , %f , %f \n", q.w(), q.x(), q.y(), q.z());

            avg_dur_build_hashmap = std::chrono::duration_cast<std::chrono::microseconds>(total_dur_build_hashmap).count() / 1000.0 / opt_count;
            std::cout << "average duration ms : build hashmap : " << avg_dur_build_hashmap << std::endl;
            avg_dur_KNN_search = std::chrono::duration_cast<std::chrono::microseconds>(total_dur_KNN_search).count() / 1000.0 / KNN_count;
            std::cout << "average duration ms : KNN search    : " << avg_dur_KNN_search << std::endl;
            avg_dur_one_frame = std::chrono::duration_cast<std::chrono::microseconds>(total_dur_one_frame).count() / 1000.0 / opt_count;
            std::cout << "average duration ms : one frame     : " << avg_dur_one_frame << std::endl;

            printf("gpu_local_map_scan_idx_set.size() : %d \n", gpu_local_map_scan_idx_set.size());
            printf("max_num_scan_in_gpu_local_map     : %d \n", max_num_scan_in_gpu_local_map    );
